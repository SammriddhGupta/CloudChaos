###############################################
# To deploy preprocessing-1 #
###############################################

# Tells Terraform to run build.sh when any of these file below changed
# - path.module is the location of this .tf file
resource "null_resource" "build_preprocessing" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/preprocessing/build.sh"
  }
}
...
data "archive_file" "preprocessing" {
  type        = "zip"
  output_path = "${path.module}/../code/preprocessing.zip" # 
  source_dir  = "${path.module}/../code/preprocessing"     # 

  depends_on = [
    null_resource.build_preprocessing # 
  ]
}
...
resource "aws_lambda_function" "preprocessing" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_preprocessing" # 
  handler       = "preprocessing_handler.lambda_handler"
  runtime       = "python3.9" # 
  timeout       = 60
...
resource "aws_lambda_permission" "preprocessing" {
...
resource "aws_apigatewayv2_integration" "preprocessing" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.preprocessing.invoke_arn # 
...
resource "aws_apigatewayv2_route" "preprocessing" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/preprocess-1" # 

  target = "integrations/${aws_apigatewayv2_integration.preprocessing.id}" # 
...
resource "aws_cloudwatch_log_group" "list_preprocessing_log" {
