###############################################
# To deploy preprocessing-final
###############################################

# Tells Terraform to run build.sh when any of these file below changed
# - path.module is the location of this .tf file
resource "null_resource" "build_preprocessing-final" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/preprocessing_final/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "preprocessing-final" {
  type        = "zip"
  output_path = "${path.module}/../code/preprocessing_final.zip" # 
  source_dir  = "${path.module}/../code/preprocessing_final"     # 

  depends_on = [
    null_resource.build_preprocessing-final # 
  ]
}
...
resource "aws_lambda_function" "preprocessing-final" {
  filename      = data.archive_file.preprocessing-final.output_path
  function_name = "${var.group_name}_${terraform.workspace}_preprocessing-final" # 
  handler       = "preprocessing_final_handler.lambda_handler"
  runtime       = "python3.9" # 
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.preprocessing-final.output_base64sha256 # 
...
resource "aws_lambda_permission" "preprocessing-final" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.preprocessing-final.function_name # 
...
resource "aws_apigatewayv2_integration" "preprocessing-final" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.preprocessing-final.invoke_arn # 
...
resource "aws_apigatewayv2_route" "preprocessing-final" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/preprocess-final" # 

  target = "integrations/${aws_apigatewayv2_integration.preprocessing-final.id}" # 
...
resource "aws_cloudwatch_log_group" "list_preprocessing-final_log" {
  name              = "/aws/lambda/${aws_lambda_function.preprocessing-final.function_name}" # 
  retention_in_days = 7
  lifecycle {
